...
export default function AddItems() {
  ...
  // --- NEW: REMOVE INDIVIDUAL ITEM LOGIC ---
  const handleRemoveItem = async (itemId: number) => {
    ...
    try {
      setIsActionLoading(true);
      // Calling your provided removeItem API
      await api.items.removeItem(selectedBasket.id, itemId);

      toast.success("Item removed from basket", { id: loadToast });

      // Refresh the manifest list
      const { data } = await api.items.viewByBasket(selectedBasket.id);
      setBasketItems(data.data || []);
    } catch (err) {
      toast.error(handleApiError(err), { id: loadToast });
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-1 h-full relative">
      ...
      {/* --- MODALS (Delete Basket & Final Submit) --- */}
      {basketToDelete && (
          ...
          <div className="bg-white border-4 border-red-600 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] max-w-sm w-full p-8 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 bg-red-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-red-700 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]"
              >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white border-2 border-gray-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)] max-w-sm w-full p-6 text-center">
            ...
            <p className="text-gray-500 text-xs font-bold uppercase mb-6">
              Resubmit this container for approval?
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowConfirmModal(false)}
                className="flex-1 px-4 py-3 bg-gray-100 text-gray-700 font-black text-[10px] uppercase tracking-widest hover:bg-gray-200"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isActionLoading}
                className="flex-1 px-4 py-3 bg-orange-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-orange-700"
              >
                Confirm
              </button>
            </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-900 text-white p-5 gap-4">
              ...
              <button
                ...
                disabled={isActionLoading}
                className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 px-6 py-3 text-[10px] font-black uppercase tracking-widest transition-all"
              >
            ...
            <div className="bg-white p-5 sm:p-8 border-x-2 border-b-2 border-gray-300 shadow-sm space-y-6">
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={isActionLoading}
                  className="sm:col-span-2 bg-gray-900 text-white py-4 font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all"
                >
                  Add Item to Basket
                </button>
              </form>
            ...
            {/* Existing Manifest List with Item Deletion */}
            <div className="mt-10">
              ...
              <div className="grid grid-cols-1 gap-3">
                {basketItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex justify-between items-center p-5 bg-white border-2 border-gray-300 hover:border-gray-900 transition-all group/item"
                  >
                    <div className="flex-1 min-w-0 pr-4">
                      <div className="flex items-center gap-2">
                        <p className="font-black text-xs uppercase tracking-tight text-gray-900 truncate">
                          {item.title}
                        </p>
                        {item.isUrgent && (
                          <span className="text-[7px] bg-red-600 text-white px-1.5 py-0.5 font-black uppercase tracking-tighter shrink-0">
                            URGENT
                          </span>
                        )}
                      </div>
                      <p className="text-[9px] text-gray-400 uppercase font-black tracking-tighter mt-1 truncate">
                        {item.description}
                      </p>
                      <p className="text-[9px] text-gray-900 uppercase font-black mt-0.5 tracking-widest">
                    ...
                    <button
                      ...
                      disabled={isActionLoading}
                      className="p-3 border-2 border-gray-100 text-gray-300 hover:text-red-600 hover:border-red-600 transition-all active:scale-90 bg-white"
                      title="Remove Item"
                    >
                      <svg
                        className="w-4 h-4"
